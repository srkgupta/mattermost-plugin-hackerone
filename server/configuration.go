...
func (c *configuration) IsValid() error {
	if c.HackeroneProgramHandle == "" {
		return errors.New("must have a hackerone program handle")
	}
	if c.HackeroneApiIdentifier == "" {
		return errors.New("must have a hackerone api identifier")
	}
	if c.HackeroneApiKey == "" {
		return errors.New("must have a hackerone api key")
	}
	if c.HackeronePollIntervalSeconds < 10 {
